...
log_open(const char *ident, int driver, int daemon)
{
  ...
  {
    if( isatty(STDIN_FILENO) && strstr(getenv("TERM")?:"", "xterm") )
    {
      fflush(0);
...
// QUARANTINE static
// QUARANTINE const char *
// QUARANTINE log_lut_rlookup(const log_lut_t *lut, int val)
// QUARANTINE {
// QUARANTINE   for( int i = 0; ; ++i )
// QUARANTINE   {
// QUARANTINE     if( (lut[i].val == val) || (lut[i].key == 0) )
// QUARANTINE     {
// QUARANTINE       return lut[i].key;
// QUARANTINE     }
// QUARANTINE   }
// QUARANTINE }
