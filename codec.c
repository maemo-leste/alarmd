...
encode_event(DBusMessageIter *iter, int *err, const alarm_event_t *eve,
             const char **def_args)
{
  encode_cookie   (iter, err, &eve->cookie);
  encode_time     (iter, err, &eve->trigger);
...
decode_event(DBusMessageIter *iter, int *err, alarm_event_t *eve)
{
  ...
  decode_cookie   (iter, err, &eve->cookie);
  decode_time     (iter, err, &eve->trigger);
